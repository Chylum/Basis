Shader "Hidden/Locked/.poiyomi/Poiyomi Toon URP/11f206cb032b669409480a8b313ae29d"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 hueShiftOKLab(float3 color, float shift, float selectOrShift)
			{
				...
				hue = shift * UNITY_TWO_PI + hue * selectOrShift;  // Add the hue shift
			...
			in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, float4 mainTexture)
			{
				...
				float4 debugColor = 0;
				#if POI_PASS_ADD
				debugColor = PoiLightDataSetupAdd(poiLight, poiFragData, poiMesh, poiCam, poiMods, mainTexture);
				#endif
				return debugColor;
			}
			...
			void PoiShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (poiLight.lightType == POI_POINT_SPOT)
				{
					if (3.0 == 3)
					{
						...
						#elif POI_PIPE == POI_URP
						poiLight.rampedLightMap = max(0, poiLight.nDotL);
						poiLight.finalLighting = LightingPhysicallyBased(poiLight.brdfData, poiLight.brdfDataClearCoat, poiLight.unityLight, poiMesh.normals[1], poiCam.viewDir, 1.0, false);
						#endif
						return;
						#endif
					}
				...
				#endif
				float shadowStrength = 0.859 * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#elif POI_PIPE == POI_URP
				Light mainLight = GetMainLight(TransformWorldToShadowCoord(poiMesh.worldPos), poiMesh.worldPos, poiLight.shadowMask);
				BRDFData brdfData;
				...
				float4 debugLight = PoiLightDataSetup(poiLight, poiFragData, poiMesh, poiCam, poiMods, mainTexture);
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.8704222,0.8704222,0.9786729,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.8704222,0.8704222,0.9786729,1).a;
				#else
				...
				if (2.0)
				{
					...
					float alphaMask = 1;
					#endif
					alphaMask = saturate(alphaMask * 1.0 + (0.0 ? 0.82 * - 1 : 0.82));
				...
				if (!skipShading)
				{
					...
					UNITY_LOOP for (uint lightIndex = 0; lightIndex < min(URP_FP_DIRECTIONAL_LIGHTS_COUNT, POI_MAX_VISIBLE_LIGHTS); lightIndex++)
					{
						...
						PoiLightDataSetup(poiLightAdd, poiFragData, poiMesh, poiCam, poiMods, mainTexture);
					...
					PoiLightDataSetupAdd(poiLightAdd, poiFragData, poiMesh, poiCam, poiMods, mainTexture);
					PoiShading(poiLightAdd, poiFragData, poiMesh, poiCam);
					PoiLightingDependantModules(poiLightAdd, poiFragData, poiMesh, poiCam, poiMods);
					poiLightOut.totalLighting += poiLightAdd.finalLighting;
					poiLightOut.totalLightAdd += poiLightAdd.finalLightAdd;
 ...
 #define OPTIMIZER_ENABLED 
			...
			#define POI_PIPE POI_URP
			#pragma target 5.0
			#define POI_PASS_DEPTH_NORMALS 1
			#pragma skip_variants FOG_LINEAR FOG_EXP FOG_EXP2
			#if POI_PIPE == POI_URP
			#define FOG_LINEAR 0
			#define FOG_EXP 0
 ...
 #define OPTIMIZER_ENABLED 
			...
			#endif
			#pragma multi_compile_instancing
			#define POI_PASS_MOTION_VECTORS 1
			#pragma skip_variants LIGHTMAP_ON DYNAMICLIGHTMAP_ON LIGHTMAP_SHADOW_MIXING SHADOWS_SHADOWMASK DIRLIGHTMAP_COMBINED _MIXED_LIGHTING_SUBTRACTIVE
			#pragma skip_variants DECALS_OFF DECALS_3RT DECALS_4RT DECAL_SURFACE_GRADIENT _DBUFFER_MRT1 _DBUFFER_MRT2 _DBUFFER_MRT3
			#pragma skip_variants _ADDITIONAL_LIGHT_SHADOWS
			#pragma skip_variants _SCREEN_SPACE_OCCLUSION
